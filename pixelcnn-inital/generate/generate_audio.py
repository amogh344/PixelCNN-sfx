...
def generate_audio(length=16000 * 10, device="cpu", model_path="pixelcnn.pt", out_file="generated.wav", sample_rate=16000):
    ...
    print(f"🎼 Generating {length} samples (~{length // sample_rate} seconds)...")
    ...
    waveform = mu_law_decode(sequence[0].cpu()).unsqueeze(0)  # shape: (1, length)
